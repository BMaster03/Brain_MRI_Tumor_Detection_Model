...
class BrainTumorDetector:
    def __init__(self, dataset_name="brain_tumor_dataset", split_ratio=0.8, batch_size=32, epochs=25):
        self.dataset_name = dataset_name
        self.split_ratio = split_ratio
        self.batch_size = batch_size
        self.epochs = epochs

        # Llamar a la función que organiza y divide el dataset
    ...
    def organize_and_split_dataset(self):
        """
        Organiza, divide y mueve el dataset en las carpetas Train, Validation y Test,
        asegurando que no se sobrescriban imágenes existentes.
        """
        ...
        # Verificar si el dataset ya está dividido
        if all((train_dir / label).exists() and any((train_dir / label).iterdir()) for label in ["YES", "NO"]) and \
           ...
           all((test_dir / label).exists() and any((test_dir / label).iterdir()) for label in ["YES", "NO"]):
            print("Las carpetas Train, Validation y Test ya existen y contienen datos.")
            return
        ...
        for label in ["YES", "NO"]:
            ...
            self._move_files(train_paths, train_dir / label)
            self._move_files(val_paths, val_dir / label)
            self._move_files(test_paths, test_dir / label)

        print(f"Dataset organizado en: {train_dir}, {val_dir}, {test_dir}")

    def _move_files(self, files, target_dir):
        """
    ...
    def build_resnet50(self):
        ...
        model = tf.keras.Sequential([ 
            # Capa convolucional adicional al principio
            tf.keras.layers.Conv2D(64, (3, 3), activation='relu', padding='same', input_shape=(224, 224, 3)), 
            tf.keras.layers.MaxPooling2D((2, 2)),  # MaxPooling después de la convolución

            base_model,
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.GlobalAveragePooling2D(),
            tf.keras.layers.Dense(256, activation='relu'), # Capa densa 
            #tf.keras.layers.Dense(128, activation='relu'), # Capa densa adicional
    ...
    def train_model(self, model, train_dir, val_dir):
        """
        Entrena el modelo utilizando las imágenes de las carpetas Train y Validation.
        """
